...
// Comma-separated list of Google emails allowed to edit/delete (empty = any signed-in user)
const ALLOWED_EDIT_EMAILS = (process.env.ALLOWED_EDIT_EMAILS || '')
  .split(',')
...
// Auth middleware: require logged-in user for edit/delete
function requireAuth(req, res, next) {
  if (req.isAuthenticated && req.isAuthenticated()) {
    return next();
  }
  return res.status(401).json({ error: 'Sign in with Google to edit or remove data.' });
}

// Only certain Google accounts can edit/delete (if ALLOWED_EDIT_EMAILS is set)
function canEdit(user) {
  if (!user || !user.email) return false;
  const email = user.email.toLowerCase();
  if (ALLOWED_EDIT_EMAILS.length === 0) return true;
  return ALLOWED_EDIT_EMAILS.includes(email);
}

function requireAllowedUser(req, res, next) {
  if (!canEdit(req.user)) {
    return res.status(403).json({ error: 'Your account is not allowed to edit or remove data.' });
  }
  next();
}
...
app.get('/api/me', (req, res) => {
  if (req.isAuthenticated && req.user) {
    return res.json({ user: req.user, canEdit: canEdit(req.user) });
  }
...
// Update entry by ID (requires allowed Google account)
app.put('/api/data/:type/:id', requireAuth, requireAllowedUser, (req, res) => {
  const type = req.params.type;
  const id = req.params.id;
  const typeConfig = scoutingConfig.scoutingTypes[type];
  if (!typeConfig) {
...
// Delete entry by ID (requires allowed Google account)
app.delete('/api/data/:type/:id', requireAuth, requireAllowedUser, (req, res) => {
  const type = req.params.type;
  const id = req.params.id;
  const typeConfig = scoutingConfig.scoutingTypes[type];
